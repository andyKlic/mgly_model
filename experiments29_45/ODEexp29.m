function dConcdt = ODEexp29(t,C,Par)
...
%expno = 29;
%Cytosolic pH
pH_cy = ((t <= 1) | (t>1 & addbuffer==0)).*pH_calc + ...
...
%  fracA = 2.000000e-003 ;
expno= Par(100);
fracA = ~(expno==45)*Par(2) + (expno==45)*((t < 40)*0.001 + ...
    (t >= 40 && t < 80)*0.004+(t >= 80 && t < 100)*0.01 + (t >= 100)*0.04);
...
%  VmaxATPase = 0 ;
VmaxATPase = (~(expno==5)*(~(expno==45)+(expno==45)*(t>=110)) + ...
    (expno==5)*(t>30))*Par(88);
...
dNavgATPdmg.*ATP+dNavgADPdmg.*ADP+dNavgAMPdmg.*AMP+dNavgPCRdmg.*PCr+ ...
...
RHSterm2_mgODE = (mgATP2.*dATPdt+mgADP.*dADPdt+mgAMP.*dAMPdt+ ...
...
fixmg = 1;
dmgdt = (t<=1).*fixmg.*(RHSterm2_mgODE+RHSterm1_mgODE.*pHODEterm1)./ ...
        (1-RHSterm1_mgODE.*pHODEterm2) + (~(t<=1)).*fixmg.*(RHSterm2_mgODE);
%fixpH = 1;
fixpH = Par(99);
