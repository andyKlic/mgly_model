function Par = PARexp45defaultfun()

Vfgly = 5.000000e-002 ; 
Par(1) = 1.9*Vfgly;
fracA = 2.000000e-003 ; 
Par(2) = fracA;
KgpA_glyf = 1.700000e-003 ; 
Par(3) = KgpA_glyf;
KgpA_pi = 4.000000e-003 ; 
Par(4) = KgpA_pi;
KgpA_igly = 2.000000e-003/1.5 ; 
Par(5) = KgpA_igly;
KgpA_ipi = 4.700000e-003 ; 
Par(6) = KgpA_ipi;
KgpA_glyb = 1.500000e-004 ; 
Par(7) = KgpA_glyb;
KgpA_g1p = 2.700000e-003 ; 
Par(8) = KgpA_g1p;
KgpA_ig1p = 1.010000e-002 ; 
Par(9) = KgpA_ig1p;
KgpB_pi = 2.000000e-004 ; 
Par(10) = KgpB_pi;
KgpB_ipi = 4.600000e-003 ; 
Par(11) = KgpB_ipi;
KgpB_iglyf = 1.500000e-002 ; 
Par(12) = KgpB_iglyf;
KgpB_g1p = 1.500000e-003 ; 
Par(13) = KgpB_g1p;
KgpB_ig1p = 7.400000e-003 ; 
Par(14) = KgpB_ig1p;
KgpB_iglyb = 4.400000e-003 ; 
Par(15) = KgpB_iglyb;
Kgp_amp = 0.00266054976219;%9.700000e-005 ; 
Par(16) = Kgp_amp;
interactioncoeff = 2.000000e-002 ; 
Par(17) = interactioncoeff;
nH = 1.75052404946130;%1.750000e+000 ; 
Par(18) = nH;
Vffpglm  = 4.800000e-001 ; 
Par(19) = 1.9*Vffpglm;
Kpglm_g1p = 6.300000e-005 ; 
Par(20) = Kpglm_g1p;
Kpglm_g6p = 3.000000e-005 ; 
Par(21) = Kpglm_g6p;
Vbbpgi = 8.800000e-001 ; 
Par(22) = 1.9*Vbbpgi;
Kpgi_g6p = 4.800000e-004 ; 
Par(23) = Kpgi_g6p;
Kpgi_f6p = 1.190000e-004 ; 
Par(24) = Kpgi_f6p;
Vffpfk  = 5.600000e-002 ; 
Par(25) = 1.9*Vffpfk;
Kpfk_f6p  = 1.800000e-004 ; 
Par(26) = Kpfk_f6p;
Kpfk_f6pT = 2.000000e-002 ; 
Par(27) = Kpfk_f6pT;
Kpfk_atp = 8.000000e-005 ; 
Par(28) = Kpfk_atp;
Kpfk_atpT = 2.500000e-004 ; 
Par(29) = Kpfk_atpT;
Kpfk_fbp = 4.020000e-003 ; 
Par(30) = Kpfk_fbp;
Kpfk_fbpT = 4.020000e-003 ; 
Par(31) = Kpfk_fbpT;
Kpfk_adp = 2.700000e-003 ; 
Par(32) = Kpfk_adp;
Kpfk_adpT = 2.700000e-003 ; 
Par(33) = Kpfk_adpT;
Kpfki = 8.700000e-004 ; 
Par(34) = Kpfki;
Kmpfk = 6.000000e-005 ; 
Par(35) = Kmpfk;
d = 1.000000e-002 ; 
Par(36) = d;
e = 1.000000e-002 ; 
Par(37) = e;
Lo = 13 ; 
Par(38) = Lo;
Vffald = 0.01065914307349;%1.040000e-001 ; 
Par(39) = 1.9*Vffald;
Kald_fbp = 5.000000e-005 ; 
Par(40) = Kald_fbp;
Kald_dhap = 2.000000e-003 ; 
Par(41) = Kald_dhap;
Kald_gap = 1.000000e-003 ; 
Par(42) = Kald_gap;
Vfftpi  = 12 ; 
Par(43) = 1.9*Vfftpi;
Ktpi_gap = 3.200000e-004 ; 
Par(44) = Ktpi_gap;
Ktpi_dhap = 6.100000e-004 ; 
Par(45) = Ktpi_dhap;
Vbbg3pdh = 8.250000e-002 ; 
Par(46) = 1.9*Vbbg3pdh;
Kg3pdh_g3p = 1.800000e-004 ; 
Par(47) = Kg3pdh_g3p;
Kg3pdh_nad = 1.200000e-005 ; 
Par(48) = Kg3pdh_nad;
Kg3pdh_dhap = 2.200000e-004 ; 
Par(49) = Kg3pdh_dhap;
Kg3pdh_nadh = 8.000000e-006 ; 
Par(50) = Kg3pdh_nadh;
Vffgad = 1.265000e+000 ; 
Par(51) = 1.9*Vffgad;
Kgapdh_gap = 2.500000e-006 ; 
Par(52) = Kgapdh_gap;
Kgapdh_nad = 9.000000e-005 ; 
Par(53) = Kgapdh_nad;
Kgapdh_pi = 2.900000e-004 ; 
Par(54) = Kgapdh_pi;
Kgapdh_bpg = 8.000000e-007 ; 
Par(55) = Kgapdh_bpg;
Kgapdh_nadh = 3.300000e-006 ; 
Par(56) = Kgapdh_nadh;
Vbbpgk = 1.120000e+000 ; 
Par(57) = 1.9*Vbbpgk;
Kpgk_bpg = 2.000000e-003 ; 
Par(58) = Kpgk_bpg;
Kpgk_adp = 8.000000e-006 ; 
Par(59) = Kpgk_adp;
Kpgk_3pg = 1.200000e-003 ; 
Par(60) = Kpgk_3pg;
Kpgk_atp = 3.500000e-004 ; 
Par(61) = Kpgk_atp;
Vffpgm = 1.120000e+000 ; 
Par(62) = 1.9*Vffpgm;
Kpgm_3pg = 2.000000e-004 ; 
Par(63) = Kpgm_3pg;
Kpgm_2pg = 1.400000e-005 ; 
Par(64) = Kpgm_2pg;
Vffen = 1.920000e-001 ; 
Par(65) = 1.9*Vffen;
Ken_2pg = 1.000000e-004 ; 
Par(66) = Ken_2pg;
Ken_pep = 3.700000e-004 ; 
Par(67) = Ken_pep;
Vffpk = 1.440000e+000 ; 
Par(68) = 1.9*Vffpk;
Kpk_pep = 8.000000e-005 ; 
Par(69) = Kpk_pep;
Kpk_adp = 3.000000e-004 ; 
Par(70) = Kpk_adp;
Kpk_pyr = 7.050000e-003 ; 
Par(71) = Kpk_pyr;
Kpk_atp = 1.130000e-003 ; 
Par(72) = Kpk_atp;
Vffldh = 1.920000e+000 ; 
Par(73) = 1.9*Vffldh;
Kldh_pyr = 3.350000e-004 ; 
Par(74) = Kldh_pyr;
Kldh_nadh  = 2.000000e-006 ; 
Par(75) = Kldh_nadh;
Kldh_lac = 1.700000e-002 ; 
Par(76) = Kldh_lac;
Kldh_nad = 8.490000e-004 ; 
Par(77) = Kldh_nad;
VforCK = 5.000000e-001 ; 
Par(78) = 1.9*VforCK;
Kck_pcr = 1.110000e-003 ; 
Par(79) = Kck_pcr;
Kck_iatp = 3.500000e-003 ; 
Par(80) = Kck_iatp;
Kck_iadp = 1.350000e-004 ; 
Par(81) = Kck_iadp;
Kck_ipcr = 3.900000e-003 ; 
Par(82) = Kck_ipcr;
Kck_cr = 3.800000e-003 ; 
Par(83) = Kck_cr;
Vfadk = 8.800000e-001 ; 
Par(84) = 1.9*Vfadk;
Kadk_amp = 3.200000e-004 ; 
Par(85) = Kadk_amp;
Kadk_atp = 2.700000e-004 ; 
Par(86) = Kadk_atp;
Kadk_adp = 3.500000e-004 ; 
Par(87) = Kadk_adp;
VmaxATPase = 5e-3 ; 
Par(88) = VmaxATPase;
Katp_ATPase = 1.000000e-004 ; 
Par(89) = Katp_ATPase;
I = 1.000000e-001 ; 
Par(90) = I;
tris = 1.500000e-002 ; 
Par(91) = tris;
carnosine = 2.500000e-002 ; 
Par(92) = carnosine;
acetate = 1e-2; 
Par(93) = acetate;
T = 310.15;% K; %Experimental temperature    
Par(94) = T;
mgT = 0.005;% total Mg++ 
Par(95) = mgT;
k = 0.08;% total K+ 
Par(96) = k;
addbuffer = 1;% dimensionless; 
Par(97) = addbuffer;
pHstat = 7.3 ;%%dimensionless;  
Par(98) = pHstat;
fixpH = 0;
Par(99) = fixpH;
expno = 45;
Par(100) = expno;
